...
# name, so I try to account for that.

DEVICE=$(ip link | grep "state UP" | awk '{ print substr($2, 1, length($2)-1) }')
TYPE=$(ip link | grep "state UP" | awk '{ print substr($2, 1, length($2)-length($2)+1) }')
